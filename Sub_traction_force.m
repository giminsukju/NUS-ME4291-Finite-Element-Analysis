function Q = Sub_traction_force(node_1_coord, node_2_coord )
%
...
%    Gauss_xx.txt (1d gauss)
  ...
  for j=1:length(Gauss_point)
    ...
    T = Input_2D_traction(X,Y);  % returns 2x1 vector
    h = Input_2D_h(X,Y);
    
    sum_1 = sum_1 + Gauss_weight(j)*T*h*N1; % 2x1 vector
    sum_2 = sum_2 + Gauss_weight(j)*T*h*N2; % 2x1 vector
  end
  sum_1 = sum_1/2*L;
  sum_2 = sum_2/2*L;
